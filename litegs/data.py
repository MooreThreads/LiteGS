    ...
    def __init__(self,cameras:dict[int,PinHoleCameraInfo],frames:list[CameraFrame],downsample:int=-1,bDevice=True):
        ...
        if bDevice:
            for camera in cameras.values():
                camera.proj_matrix=torch.Tensor(camera.proj_matrix).to('musa')
            for frame in frames:
                frame.view_matrix=torch.Tensor(frame.view_matrix).to('musa')
                for key in frame.image.keys():
                    frame.image[key]=torch.tensor(frame.image[key]).to('musa')
        ...
        for frame in self.frames:
            ...
            if bDevice:
                self.frustumplanes.append(torch.Tensor(frustumplane).to('musa'))
            else:
