...
def ssim(img1, img2, window_size=11, size_average=True):
    ...
    if img1.is_musa:
        window = window.musa(img1.get_device())
...
if __name__ == "__main__":
    ...
    for _ in range(iterations):
        with torch.no_grad():
            img1_og = nn.Parameter(torch.rand([B, CH, H, W], device="musa"))
            img2_og = torch.rand([B, CH, H, W], device="musa")
    ...
    img1 = nn.Parameter(torch.rand([B, CH, H, W], device="musa"))
    img2 = torch.rand([B, CH, H, W], device="musa")
