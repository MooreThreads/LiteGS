    ...
    if (tile_id != 0 && tile_id < start_index.size(1) - 1)
    {
        ...
        if (start_index_in_tile != -1)
        {
            ...
            for (int i = 0; i < PIXELS_PER_THREAD; i++)
            {
                ...
                shared_img_grad[0][i][threadIdx.y * blockDim.x + threadIdx.x] = d_img[batch_id][0][tile_id - 1][in_tile_y + i][in_tile_x];
                shared_img_grad[1][i][threadIdx.y * blockDim.x + threadIdx.x] = d_img[batch_id][1][tile_id - 1][in_tile_y + i][in_tile_x];
                shared_img_grad[2][i][threadIdx.y * blockDim.x + threadIdx.x] = d_img[batch_id][2][tile_id - 1][in_tile_y + i][in_tile_x];
                if (enable_trans_grad)
                {
                    shared_img_grad[3][i][threadIdx.y * blockDim.x + threadIdx.x] = d_trans_img[batch_id][0][tile_id - 1][in_tile_y + i][in_tile_x];
                }
